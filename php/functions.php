	...
	function get_comments() {
		...
		foreach($result as $item) {
			...
			echo '<div class="comment" id="'.$par_code.'">'
					...
					.'<a class="link-reply" id="reply" name="'.$par_code.'">Відповісти</a>';
				...
				if($chi_cnt == 0){
					...
					echo '<a class="link-reply" id="children" name="'.$par_code.'"><span id="tog_text">Відповіді</span> ('.$chi_cnt.')</a>'
						.'<div class="child-comments" id="C-'.$par_code.'">';

					foreach($chi_result as $com) {
						$chi_date = new dateTime($com['date']);
						$chi_date = date_format($chi_date, 'd.m.Y | H:i:s');
						$chi_user = $com['user'];
						$chi_com = $com['text'];
